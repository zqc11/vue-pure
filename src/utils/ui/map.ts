...
import { MapGenerator } from "##/shared/mapExport";
...
export const captureMapScreen = async (
  ...
  let canvasWH = Math.min(canvas.width, canvas.height, imgWidth ?? 300);
  var mapGen = new MapGenerator(map as any, [canvasWH, canvasWH]);
  let base64 = await mapGen.toBase64(maxWidth);
  return base64;
};
