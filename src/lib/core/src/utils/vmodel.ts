...
export default function <T, U = Ref<T> | UnwrapRef<T>>(
  propertyValue: Record<string, any>,
  eventName: string | string[] = "change",
  modelName = "value"
): U {
  const instance = getCurrentInstance();
  const eventNames = eventName instanceof Array ? eventName : [eventName];

  if (!instance) throw new Error("vmodel: Instance not found");
  ...
  watch(instance.props, () => {
    if (
      isPrimitive(instance.props[modelName])
