...
export const parentsNameIs = (instance: any, parentName: string) => {
  ...
  // Vue 3
  if (instance.parent.type) return instance.parent.type.name === parentName;
...
export const parentNameContains = (
  instance: any,
  parentNameFragment: string
): boolean => {
  if (!instance.parent) return false;
...
export const duplicateEvents = <T extends InstanceWithOn<any>>(
  object: T,
