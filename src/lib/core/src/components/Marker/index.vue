...
import { Marker, PointLike, Anchor, Alignment } from "vjmap";
import Deferred from "/@/lib/core/src/utils/deferred";
import injectMap from "/@/lib/core/src/shared/mapInject";
...
export default defineComponent({
  ...
  setup(props) {
    const { vmap } = injectMap.setup();
    const vmarker = new Deferred<Marker>();

    const icon = ref(null) as Ref<any | null>;

    const options = getMarkerOptions(props);

    provide("vmarker", vmarker);

    onMounted(async () => {
      const instance = getCurrentInstance();
      if (instance && vmap) {
        await mountMarker(options, vmap, vmarker, instance, props.lngLat, icon);
